...
type spp =
  ...
  | Drop
  | Union of
      field * spp ref Value.M.t Value.M.t * spp ref Value.M.t * spp ref
  ...
  | Union (f, fms, ms, d) ->
      let branches =
        ...
        |> List.map (fun (vi, m) ->
               let mods =
                 Value.M.bindings m
                 |> List.map (fun (vj, spp) ->
                        Nk.seq_pair (Nk.modif f vj) (to_exp_inner spp))
                 |> Nk.union
               in
      ...
      let mods =
        ...
        |> List.map (fun (vi, sppi) ->
               Nk.seq_pair (Nk.modif f vi) (to_exp_inner sppi))
        |> Nk.union
      in
      let def =
        Nk.seq_pair
...
let rec to_sp_bwd_inner (spp : spp) : Sp.t =
  match spp with
  | Skip -> Sp.Skip
  | Drop -> Sp.Drop
  | Union (f, fms, ms, d) ->
      let ms1 =
        Value.M.map
          (fun ms ->
            List.map
              (fun (_, sppi) -> to_sp_bwd_inner sppi)
              (Value.M.bindings ms)
            |> Sp.union)
          !!!fms
      in
      ...
      in
      Sp.mk (f, Value.right_join Sp.Drop ms1 ms2, to_sp_bwd_inner !d)

let to_sp_bwt spp = to_sp_bwd_inner !spp
...
let union_pair =
  let union_pair self spp1ref spp2ref =
    ...
    | _, Drop -> spp1ref
    | Skip, Skip -> skip
    | Skip, Union (f, _, _, _) -> self (dummy f skip) spp2ref
    | Union (f, _, _, _), Skip -> self spp1ref (dummy f skip)
    | Union (f1, fms1, ms1, d1), Union (f2, fms2, ms2, d2) ->
...
let intersect_pair =
  let intersect_pair self spp1ref spp2ref =
    ...
    | Drop, _ | _, Drop -> drop
    | Skip, Skip -> skip
    | Skip, Union (f, fms, ms, d) -> self (dummy f skip) spp2ref
    | Union (f, fms, ms, d), Skip -> self spp1ref (dummy f skip)
    | Union (f1, fms1, ms1, d1), Union (f2, fms2, ms2, d2) ->
        if f1 < f2 then self spp1ref (dummy f1 spp2ref)
        else if f2 < f1 then self (dummy f2 spp1ref) spp2ref
        else
...
let rec push_inner (sp : Sp.t) (spp : spp) =
  match (sp, spp) with
  ...
  | Sp.Skip, Union (f, fms, ms, d) ->
      let thru_fms =
        List.map
          (fun (v, vms) ->
            Value.M.mapi (fun v spp -> push_inner Sp.Skip spp) vms)
          (Value.M.bindings !!!fms)
        |> Value.map_op Sp.drop Sp.union_pair
      in
      let thru_ms = Value.M.mapi (fun v spp -> push_inner Sp.Skip spp) !!ms in
      ...
      in
      let push_default = push_inner Sp.Skip !d in
      let diffkeys =
  ...
  | Union (f1, fms1, d1), Union (f2, fms2, ms2, d2) ->
      if f1 < f2 then
        let fms = Value.M.mapi (fun v spi -> push_inner spi spp) fms1 in
        Sp.mk (f1, fms, push_inner d1 spp)
      else if f2 < f1 then
        let fmsA =
          List.map
            (fun (_, muts) ->
              let tsts =
                Value.M.mapi (fun v sppi -> push_inner sp sppi) muts
              in
        ...
        let ms =
          Value.right_join Sp.drop
            ...
            (Value.M.map (fun sppi -> push_inner sp sppi) !!ms2)
        in
        let fmsB = Sp.mk (f2, ms, push_inner sp !d2) in
        ...
        let pkA =
          List.map
            (fun (v, spi) ->
              ...
              | None -> (
                  let tsts =
                    Value.M.map (fun sppj -> push_inner spi sppj) !!ms2
                  in
                  match Value.M.find_opt v ms2 with
                  | None ->
                      let tsts' = Value.M.add v (push_inner spi !d2) tsts in
                      Sp.mk (f1, tsts', Sp.drop)
                  | Some sppj -> Sp.mk (f1, tsts, Sp.drop))
              | Some m ->
                  let tsts =
                    Value.M.map (fun sppj -> push_inner spi sppj) !!m
                  in
                  Sp.mk (f1, tsts, Sp.drop))
          ...
          List.map
            (fun (v, sppi) ->
              if Value.M.mem v fms1 then Sp.drop
                ...
                let tsts = Value.M.map (fun sppj -> push_inner d1 sppj) sppi in
                Sp.mk (f1, tsts, Sp.drop))
            (Value.M.bindings !!!fms2)
          |> Sp.union
        in
        let ms =
          ...
          |> Value.left_join Sp.drop
               (Value.M.map (fun sppi -> push_inner d1 sppi) !!ms2)
        in
        let pkC = Sp.mk (f1, ms, push_inner d1 !d2) in
        Sp.union [ pkA; pkB; pkC ]

let push sp spp = push_inner sp !spp
let pull (spp : t) (sp : Sp.t) = seq_pair spp (of_sp sp) |> to_sp_bwt
