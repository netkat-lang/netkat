...
let rec compare spp1 spp2 = match spp1, spp2 with
  ...
  | Union (f1,fms1,ms1,d1), Union (f2,fms2,ms2,d2) ->
      ...
      else let cmp_fms = Value.M.compare (Value.M.compare compare) fms1 fms2 in
           if cmp_fms < 0 then -1
           else if cmp_fms > 0 then 1
           else let cmp_ms = Value.M.compare compare ms1 ms2 in
                if cmp_ms < 0 then -1
...
let mk (f, fms, ms, d) = (*Union (f, fms, ms, d)*)
  ...
  let fms'' = List.fold_left (fun m (vi, sppi) ->
    if eq sppi Drop && not (Value.M.mem vi fms') then
      Value.M.remove vi m else m) fms' (Value.M.bindings ms) in
  let ms' = Value.M.filter (fun _ sppi -> not (eq sppi Drop)) ms in
  let fms''' = Value.M.filter (fun vi mi ->
    let drop_branch = if Value.M.mem vi ms' || eq d Drop then
      ms' else Value.M.add vi d ms' in
    not (Value.M.equal eq mi drop_branch)) fms'' in
  if Value.M.is_empty fms''' && Value.M.is_empty ms' then
    d
  else
    Union (f, fms''', ms', d)
...
let rec to_sp_bwd (spp: t) : Sp.t = match spp with
  ...
  | Drop -> Sp.Drop
  | Union (f, fms, ms, d) ->
      let ms1 = Value.M.map (fun ms -> List.map (fun (_,sppi) -> (to_sp_bwd sppi))
                                        (Value.M.bindings ms) |> Sp.union) fms in
      let y = List.map (fun (_,sppi) -> to_sp_bwd sppi) (Value.M.bindings ms)
              |> Sp.union in
...
let vm_to_string (m: Sp.t Value.M.t): string =
  List.map (fun (vj, sp) ->
      Value.string_of_val vj ^ "↦" ^ (Sp.to_string sp)) (Value.M.bindings m)
        |> String.concat ", "

let vmpp_to_string (m: t Value.M.t): string =
  List.map (fun (vj, sp) ->
      Value.string_of_val vj ^ "↦" ^ (to_string sp)) (Value.M.bindings m)
        |> String.concat ", "

let vmm_to_string (m: t Value.M.t Value.M.t): string = 
  List.map (fun (vi, mi) ->
    (Value.string_of_val vi) ^ "--->" ^ (vmpp_to_string mi)
    ) (Value.M.bindings m) |> String.concat "; "

let get_branch (x: t) (v: value) : t Value.M.t =
  match x with
  | Drop -> Value.M.empty
  | Skip -> Value.M.singleton v Skip
  | Union (f,fms,ms,d) -> match Value.M.find_opt v fms with
                          | Some m -> m
                          | None -> if Value.M.mem v ms ||  eq d Drop then
                                      ms
                                    else
                                      Value.M.add v d ms
  ...
  | Union (f1,fms1,ms1,d1), Union (f2,fms2,ms2,d2) ->
      if f1 < f2 then
        ...
        (* XXX As-yet unadapted section here:
          ...
          *)
        let keyset = Value.(S.union (union_keys [fms1; fms2]) (union_keys [ms1; ms2])) in
        let fms = Value.S.fold (fun v m ->
      ...
      if f1 < f2 then
        ...
        let matchup (vi, sppi) =
          ...
          |> List.fold_left (fun m (vj, sppj) ->
            Value.M.add vj (seq_pair sppi sppj) m) Value.M.empty in
        let msA = union_maps (List.map matchup (Value.M.bindings ms1)) in
        let keyset = Value.(S.union (union_keys [fms1; fms2]) (union_keys [ms1; ms2; msA])) in
        let fms = Value.S.fold (fun v m -> 
          ...
          let res = List.map (fun (vi, sppi) ->
            ...
            List.fold_left (fun mi (vj, sppj) ->
              Value.M.add vj (seq_pair sppi sppj) mi) Value.M.empty spp_v_i) spp_v in
...
let rec push (sp: Sp.t) (spp: t) = match sp, spp with
  ...
  | Sp.Skip, Union (f, fms, ms, d) ->
      let thru_fms = List.map (fun (v,vms) ->
          Value.M.mapi (fun v spp -> push Sp.Skip spp) vms) (Value.M.bindings fms)
        |> Value.map_op Sp.drop Sp.union_pair in
      ...
      let matched_values = Value.(S.union (keys fms) (keys ms)) in

      (* Next we need to identify the right slice of [Skip] to push through [d],
         i.e. whatever doesn't match [fms] or [ms]. *)
      let branchesD,unmatched_vs = Value.S.fold (fun v (m,uvs) -> 
      ...
      let diffkeys = Value.(S.diff (keys branchesD) matched_values)
                 |> Value.S.elements in
