...
(** [rep_over diversify sp fields] is like [rep], but returns one packet per
    live combination of [diversify] fields in [sp] (bounding the resulting
    list to the product of branching factors of exactly those fields),
    while still picking a single arbitrary representative for every other
    field, as [rep] does. Finds every live combination via a full traversal
    (see [restrict_over]), so a diversify field's value is never missed
    just because it sits behind a branch of some unrelated field. With
    [diversify] empty, this is [rep] wrapped in a singleton list. *)
