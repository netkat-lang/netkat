...
(** Runs a word on the DFA and returns whether it is accepted. *)
val accept : t -> word -> bool

(** Returns whether a DFA accepts all of the words in the first list and rejects
    all of the words in the second list. Returns [false] if either condition fails
    on any string. *)
val validate : t -> word list -> word list -> bool

    (** Returns [true] if and only if the DFA recognizes the empty language. *)
val is_empty : t -> bool

(** Returns a word in the language of the DFA. *)
val rep : t -> word

(** Returns a DFA that recognizes the complement of the language of the given DFA. *)
val complement : t -> t

(** Writes the table for the DFA to stdout. *)
val print : t -> unit
...
(** Converts the DFA to an NFA which has the same transitions and accepting states. *)
val to_nfa : t -> IntNfa.t

(** Performs DFA minimization. *)
val minimize : t -> t

(** Constructs a DFA from a Regular Expression. An alphabet is required since a Regular Expression
...
(** Constructs a Regular Expression representing the language of the DFA. *)
val to_rx : t -> Rx.t

(** Constructs a DFA recognizing the union of the languages of two DFAs. *)
val union : t -> t -> t

(** Constructs a DFA recognizing the intersection of the languages of two DFAs. *)
val intersect : t -> t -> t

(** Constructs a DFA recognizing the language of the first DFA minus the language of
   the second DFA. *)
val diff : t -> t -> t
...
(** Constructs a DFA recognizing all strings which are either accepted by both
