...
(** [concat w1 w2] returns the concatenation of [w1] and [w2], i.e. [w1 . w2].
...
(** Converts a word to a list of ints. *)
val to_intlist : t -> int list

(** Converts a word to its string representation (for output). The alphabet's string
    representation for each symbol is used. *)
val to_string : Alphabet.t -> t -> string

(** Converts to a list of [Alphabet.symbol]. *)
val to_symlist : t -> Alphabet.symbol list

(** Converts from a list of [Alphabet.symbol]. *)
